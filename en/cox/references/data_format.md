# Observability Data Format Specifications
...
```json
{
  ...
  "iterations": [
    {
      "iteration_id": "Iteration Unique Identifier",
      ...
      "modules": [
        {
          "module_id": "Module Unique Identifier",
      ...
      "assumptions": [
        {
          "assumption_id": "Assumption Unique Identifier",
...
| current_iteration | string | Yes | Current iteration ID |
| iterations | array | Yes | Iteration list |
| iterations[].iteration_id | string | Yes | Iteration unique identifier |
...
| iterations[].tasks | array | Yes | Task list |
| iterations[].tasks[].task_id | string | Yes | Task unique identifier |
...
| iterations[].assumptions | array | Yes | Assumption list |
| iterations[].assumptions[].assumption_id | string | Yes | Assumption unique identifier |
  ...
  "iterations": [
    {
      ...
      "tasks": [
        {
          "task_id": "TASK-001",
          "task_name": "Refactor Order Creation API",
          "status": "done",
          "assignee": "Zhang San",
          "priority": "high",
          "tags": ["backend", "api"]
        ...
        {
          "task_id": "TASK-002",
          "task_name": "Optimize Order Payment Flow",
          "status": "in_progress",
          "assignee": "Li Si",
          "priority": "critical",
    ...
    },
    {
      ...
      "tasks": [
        {
          "task_id": "TASK-003",
          "task_name": "Implement Product Search Feature",
          "status": "todo",
          "assignee": "Wang Wu",
          "priority": "high",
          "tags": ["backend", "search"]
      ...
      "assumptions": [
        {
          ...
          "description": "Product count will not exceed 1 million",
          "status": "pending",
...
### Module Status Enum Values
- `pending`: Pending development
- `developed`: Developed pending confirmation
- `confirmed`: Confirmed complete
- `optimized`: Confirmed pending optimization

### Complete Example
```json
{
  "app_name": "E-commerce Platform Backend Service",
  "version": "v2.0.0",
...
```json
{
  ...
  "tracing_points": [
    {
      "point_id": "Instrumentation Point Unique Identifier",
      "module": "Belonging Module",
      "location": "Instrumentation Point Location",
      "metric_type": "Metric Type",
      "status": "Instrumentation Point Status",
      "last_verified": "Last Verification Time"
    }
  ],
  "anomalies": [
    {
      "anomaly_id": "Anomaly Unique Identifier",
      "type": "Anomaly Type",
      "severity": "Severity",
...
|------------|------|----------|-------------|
| last_updated | string | Yes | Last update time, format: YYYY-MM-DD HH:MM:SS |
| test_suites | array | Yes | Test suite list |
| test_suites[].suite_name | string | Yes | Test suite name |
| test_suites[].total_tests | number | Yes | Total test case count |
| test_suites[].passed_tests | number | Yes | Passed test count |
| test_suites[].failed_tests | number | Yes | Failed test count |
| test_suites[].skipped_tests | number | Yes | Skipped test count |
...
| tracing_points | array | Yes | Instrumentation point list |
| tracing_points[].point_id | string | Yes | Instrumentation point unique identifier |
| tracing_points[].module | string | Yes | Belonging module |
| tracing_points[].location | string | Yes | Instrumentation point location (file path:line number) |
| tracing_points[].metric_type | string | Yes | Metric type: `counter`/`gauge`/`histogram`/`summary` |
| tracing_points[].status | string | Yes | Instrumentation point status: `active`/`inactive`/`deprecated` |
| tracing_points[].last_verified | string | Yes | Last verification time, format: YYYY-MM-DD HH:MM:SS |
| anomalies | array | Yes | Anomaly list |
| anomalies[].anomaly_id | string | Yes | Anomaly unique identifier |
| anomalies[].type | string | Yes | Anomaly type: `performance`/`functional`/`integration`/`security` |
| anomalies[].severity | string | Yes | Severity: `low`/`medium`/`high`/`critical` |
...
| anomalies[].status | string | Yes | Handling status: `open`/`investigating`/`resolved`/`ignored` |
  ...
  "anomalies": [
    {
      ...
      "type": "performance",
      "severity": "high",
      "description": "Order creation API response time exceeds 2 seconds",
...
2. All enum field values must be within allowed enum ranges
3. Numeric ranges must conform to field definitions (such as completion rate 0.0-1.0)
4. Numeric relationships must conform to logic (such as passed tests <= total tests)
...
### Common Errors
1. **Enum Value Error**: status field uses value outside enum range
2. **Value Out of Range**: completion_rate greater than 1.0 or less than 0.0
3. **Date Format Error**: Date format doesn't conform to YYYY-MM-DD or YYYY-MM-DD HH:MM:SS
4. **Logic Error**: passed_tests > total_tests
5. **Missing Required Field**: Lacks required fields

### Auto-Generate Initial Data
If unsure how to create initial data, you can describe project situation to Agent, which will generate initial data files based on specifications.
